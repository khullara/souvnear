...
$bid = $_POST['bid'];
$a=1;
$sql = "Select * from products_added WHERE username = $id AND Status='W'";
$result = mysql_query($sql) or die(mysql_error());
$count = mysql_num_rows($result);
if($count > 0){
?>
<!--form action="buyer-ajax/wishlistexcel.php" method="post">
	<input type="hidden" name="userid" value="<?php //echo $id; ?>">
	<input type="hidden" name="buyerid" value="<?php //echo $bid; ?>">
	<input type="submit" name="excel_export" value="Export Excel">
</form-->
<table id="example-wishlist" class="display" cellspacing="0" border="1" style="width:100%; margin:0 auto; text-align:center;border-collapse: collapse;" >
			<thead>
			<tr>
					<th>Sr. No</th>
					<th>Buyer Name</th>
			...
			{
				echo "<p style='color:#ff0000; text-align:center;'>No Items in WishList.</p>";
			}
			?>

<script>
		$(document).ready(function() {
			$('#example-wishlist').DataTable({
				dom: 'Bfrtip',
				buttons: [
					'excel'
				]
